...
local function OnTooltipSetItem(tooltip)
	if tooltip then
		...
		if LastSeenDB.Items[itemID] then
			...
			AddTextToTooltip(tooltip, string.format("%s: |cff52D66C%s|r | |cff52D66C%s|r | |cff52D66C%s|r", LastSeen.ColoredAddOnName(), item.source or "UNK", item.map or "UNK", LastSeenDB.Items[itemID].lootDate or date(LastSeen.dateFormat)))
		end
	end
end
