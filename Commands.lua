...
SlashCmdList["LastSeen"] = function(cmd, editbox)
	...
	elseif cmd == tbl.L["COMMAND_FORMAT"] then -- Allows the player to change their date format for existing items.
		tbl.DateFormat(args);
	elseif cmd == tbl.L["COMMAND_HELP"] then -- Provides the available commands to the player in the chat frame.
		tbl.Help();
	...
	elseif cmd == tbl.L["COMMAND_MAN"] and args ~= "" then -- Allows the player to ask the addon for command usage.
		tbl.Manual(args);
